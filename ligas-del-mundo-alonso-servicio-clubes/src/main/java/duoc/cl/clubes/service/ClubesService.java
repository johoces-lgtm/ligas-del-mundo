...
public class ClubesService {
    ...
    public List<DtoClubesResponse> listarTodos() {
        ligasClient.pingLigas();
        return clubesRepository.findAll().stream().map(this::mapearAResponse).collect(Collectors.toList());
    }
    ...
    public DtoClubesResponse buscarPorId(Long id) {
        ...
        if (!ligasClient.validarLiga(club.getLigaId())) {
            throw new ResourceNotFoundException("Inconsistencia: La liga de este club ya no existe.");
        }
